...
%Since $\hat{Z}$ is determined by its weights $w_i$, we will sometimes talk about the reliability with weights $w_i$.
...
By far the most popular choice is to give the same weight to each $X_i$, that is, $w = \mathbf{i}=(1,1,\ldots,1)$. This leads to the congeneric reliability, which is the estimand of coefficient alpha under certain conditions. Its definition is
\begin{equation}
...
\begin{equation*}
\hat{Z}=w_{0}\sum_{i=1}^{k}X_{i}\label{eq:standardized sum score}
\end{equation*}
...
Correction for attenuation cannot be used unless both $ R(\hat{Z}_i)$s are the squared correlation between $\hat{Z}_i$ and $Z_i$. For example, if you used the standardized sum scores (\cref{eq:standardized sum score} (to make predictions for the first model, it would be wrong to use the congeneric reliability, as it based on the sum scores. If you used the congeneric reliability, the correlation between $Z_1$ and $Z_2$ would incorrectly be calculated according to $$\rho'= R(\hat{Z}_{1}) R(\hat{Z}_{2})\Cor(Z_{1},Z_{2})$$
instead of the correct $\rho= R(\hat{Z}_{1}^{*}) R(\hat{Z}_{2})\Cor(Z_{1},Z_{2})$.
You would be off by a factor of $ R(\hat{Z}_{1}^{*})/ R(\hat{Z}_{1})$.
...
Coefficient $H$ ($R_{H})$ & $\frac{k\overline{\lambda^{2}\sigma^{-2}}}{k\overline{\lambda^{2}\sigma^{-2}}+1}$ & $\lambda_{i}\sigma_{i}^{-2}$ & Thomson factor score \\[2ex]
...
\par\end{centering}
\vskip7.0pt
\textit{Note}: The score $\hat{Z} = w_{0}\sum_{i=1}^{k}w_iX_{i}$, where $w_0 = \overline{w\lambda}/(k\overline{w\lambda}^{2}+\overline{w^{2}\sigma^{2}})$.
\end{table}
...
$ R$, while it is hard to predict the winner among $ R$
...
\end{defn}
Also define the natural analogues of the reliabilities in \Cref{sec:coefficienta alpha}: The concrete congeneric reliability is the weighted ordinal reliabiltiy with unit weights, while the concrete ordinal \text{H} as the weighted ordinal reliability with optimal weights.  
...
\begin{eqnarray}
 R_{w}' & = & \frac{\lambda^{2}}{(k\lambda^{2}+\sigma^{2})^{2}}\mathbf{i}^{T}\Xi\mathbf{i},\label{eq:Alpha prime}\\
...
with optimal weights is $ R'_{H}=0.68$, and with equal weights
and $ R'=0.64$. But the theoretical variants are $ R'_{H}=0.81$
and $ R'=0.77$, both missed by large margin. In comparison, the
reliabilities calculated under the congeneric model with $\mathtt{lavaan}$ \citep{Rosseel2012-yg}
equals $ R'_{H}=0.77$ and $ R'=0.74$.
...
$Z$ and $\hat{Z}$,
\begin{equation}
 R=\Cor^{2}(\hat{Z},Z) = \frac{(\tsum_{i=1}^{k}w_{i}\lambda_{i})^{2}}{(\tsum_{i=1}^{k}w_{i}\lambda_{i})^{2}+\tsum_{i=1}^{k}w_{i}^{2}\sigma_{i}^{2}}.
\end{equation}
...
(iii) 
Using the same reasoning as in \cref{lem:alpha_s}, $\alpha_{S}\geq R$ if
and only if 
\begin{equation}
...
\end{equation*}
As for (ii), first assume that $\hat{\lambda}^2\neq 0$. By the invariance property of maximum likelihood and the definition of the congeneric reliability (\cref{eq:Congeneric reliability}), the maximum likelihood estimator of the congeneric reliability is
\begin{eqnarray*}
...
\end{eqnarray*}
If the maximum likelihood estimator of $\lambda^{2}$ is $0$, then $(\mathbf{i}^{T}\mathbf{\hat{\lambda}})^{2}=0$,
...
\end{equation}
with equality if and only if $x_{1}=\cdots=x_{k}$. This is a generalization of the arithmetic mean--geometric mean and harmonic mean--arithmetic mean inequalities.

Moreover, a generalized Minkowski inequality holds for the power means. See \citep[][Section 2.4, Theorem 9.]{Bullen2013-os} more proofs.
...
\end{lem}
\begin{proof}
(\Cref{eq:power mean inequality}.) By the power
...
(\Cref{eq:power mean full inequality}.) 
...
\]
This is a case of \cref{lem:power mean lemma}, \cref{eq:power mean full inequality}.
