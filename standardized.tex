...
\titletag{An arXiv preprint, v1.0}
\makeatletter
\titlerunning{\@title}
\makeatother
...
% \begin{equation}
% X_{i}^{*}=\frac{\lambda_{i}}{\sigma_{i}}Z+\epsilon_{i}\:i=1,\ldots,k\label{eq:sigma model}
% \end{equation}
...
% model $X^{*}=X\lambda_{i}(\lambda_{i}^{2}+\sigma_{i}^{2})^{-1/2}+\epsilon$,
...
% \end{eqnarray*}
% (iv) This one is similar to (ii). Just use the modified model $X^{*}=X\lambda\sigma^{-1}+\epsilon$,
