...
\end{equation}
where $\diag\Sigma$ is the diagonal matrix .
...
I emphasize we do not have to assume normality or continuous $X$s to get Proposition \ref{prop:weighted alpha}. In fact, none of the derivations in Proposition \ref{prop:weighted alpha} involve the distributions of $Z$ and $\varepsilon$. This is contrary to some claims about coefficient alpha in the literature, e.g. \citep[][p.415]{McNeish2018-vu}, \citep[][p.21]{Zumbo2007-ap} and \citep[][p. 1185]{Zumbo2019-lm}. \citet{Raykov2019-yr} lamented this widespread misunderstanding, while \citet[][p. 1060]{Chalmers2018-fj} investigated its source. That said, while continuous data is no assumption for the coefficient alpha or the congeneric reliability, the implicitly assumed congeneric model \eqref{eq:congeneric model} is unrealistic for discrete data. 
...
\begin{enumerate}[label=(\roman*)]
    ...
    $$\sigma^{2}(\alpha)= \sigma^{2}(\alpha_{s})=2\frac{k}{k-1}(1-\alpha_{s})^{2},$$
...
% that $\alpha(\Sigma)=\omega$ {[}{[}fill in{]}. Now it
...
% elements $\lambda^{2}$. Then $\lambda^{2}=(k-1)^{-1}(\mathbf{i}^{T}\Sigma\mathbf{i}/k-\tr\Sigma/k)$
...
% \begin{eqnarray}
% \alpha_{\sigma} & = & \frac{k}{k-1}(1-\frac{\tr\Xi}{\mathbf{i}^{T}\Xi\mathbf{i}})\label{eq:alpha_sigma-1}
% \end{eqnarray}
...
% \begin{equation}
% X_{i}^{*}=\frac{\lambda_{i}}{\sigma_{i}}Z+\varepsilon_{i}\:i=1,\ldots,k\label{eq:sigma model}
% \end{equation}
% is $\tau$-equivalent. In other words, $\alpha_{\sigma}=\omega_{\sigma}$
% only when every standardized factor loading is the same, or $\lambda_{i}/\sigma_{i}=\lambda^{*}$
% for some $\lambda^{*}$. From Proposition \ref{prop:Z-reliabiltiy}
...
% $\omega_{k}=k/(k+1)$ when \eqref{eq:sigma model} holds, no matter what values of $\lambda$ and $\sigma$.
...
(iv) First assume $\lambda_{i}=\sigma_{i}$ for each $i$. Then $\alpha_{s}=k/(k+1)$
...
is a $(\lambda,\omega)$ where $[\alpha_{s}-\omega](\lambda,\sigma)=0$,
...
\[
\sigma^{2}(\alpha_{s})=\frac{1}{(k-1)^{2}}\frac{1}{(1+\rho(k-1))^{4}}2\mathbf{1}^{T}Q_{p}(P\otimes P)Q_{P}^{T}\mathbf{1}.
\]
...
Thus
\[
\sigma^{2}(\alpha_{s})=\frac{2k}{(k-1)}\frac{(\rho-1)^{2}}{(1+\rho(k-1))^{2}}
\]
and the limits are equal.
Since
\begin{eqnarray*}
\alpha_{s} & = & \frac{k}{k-1}(1-\frac{k}{k(k-1)\rho+k}) = \frac{k\rho}{(k-1)\rho+1}
\end{eqnarray*}
we get
\begin{eqnarray*}
1-\alpha_{s} & = & \frac{(k-1)\rho+1-k\rho}{(k-1)\rho+1} = \frac{1-\rho}{(k-1)\rho+1}
\end{eqnarray*}
Taken together, the asymptotic variance is
\[
\sigma^{2}(\alpha_{s})=2\frac{k}{k-1}(1-\alpha_{s})^{2}
\]
...
% \begin{lem}
% \label{lem:weighted averages}Let $\{ x_{i}\} _{i=1}^{k},\{ w_{i}\} _{i=1}^{k}$
...
% \end{proof}
% \begin{proof}[Proof of the claim $w_{0} = \overline{\lambda^{2}}/(k\overline{\lambda}^{2}+\overline{\sigma^{2}})$ on page \pageref{eq:w0}]
% \label{proof:w0}
% \end{proof}
...
% \begin{eqnarray*}
% E[(Z-w\tsum_{i=1}^{k}(X_{i}-\mu_{i})-\theta)^{2}] & = & E[([1-w\tsum_{i=1}^{k}\lambda_{i}]Z-w\tsum_{i=1}^{k}\sigma_{i}\varepsilon_{i})^{2}],\\
%  & = & E[([1-\tsum_{i=1}^{k}\lambda_{i}w_{i}]Z)^{2}]+E[(w\tsum_{i=1}^{k}\sigma_{i}\varepsilon_{i})^{2}],\\
%  & = & [1-w\tsum_{i=1}^{k}\lambda_{i}]^{2}+w^{2}\tsum_{i=1}^{k}\sigma_{i}^{2}.
% \end{eqnarray*}
% Differentatiate with respect to $\omega$,
% \[
% 2\tsum_{i=1}^{k}\lambda_{i}[w\tsum_{i=1}^{k}\lambda_{i}-1]+2w\tsum_{i=1}^{k}\sigma_{i}^{2}.
% \]
% Hence $w=\overline{\lambda}/(\overline{\sigma^{2}}+k\overline{\lambda}^{2})$.
% The risk is
% \begin{eqnarray*}
% [1-wk\overline{\lambda}]^{2}+w^{2}k\overline{\sigma^{2}} & = & [1-\frac{k\overline{\lambda}^{2}}{\overline{\sigma^{2}}+k\overline{\lambda}^{2}}]^{2}+\frac{k\overline{\lambda}^{2}\overline{\sigma^{2}}}{(\overline{\sigma^{2}}+k\overline{\lambda}^{2})^{2}}\\
...
% \begin{eqnarray*}
% E[(Z-\tsum_{i=1}^{k}w_{i}(X_{i}-\mu_{i}))^{2}] & = & E[([1-\tsum_{i=1}^{k}\lambda_{i}w_{i}]Z-\tsum_{i=1}^{k}w_{i}\sigma_{i}\varepsilon_{i})^{2}]\\
%  & = & E[([1-\tsum_{i=1}^{k}\lambda_{i}w_{i}]Z)^{2}]+E[(\tsum_{i=1}^{k}w_{i}\sigma_{i}\varepsilon_{i})^{2}]\\
%  & = & [1-\tsum_{i=1}^{k}\lambda_{i}w_{i}]^{2}+\tsum_{i=1}^{k}w_{i}^{2}\sigma_{i}^{2}
% \end{eqnarray*}
...
% The risk is
% \begin{eqnarray*}
% w_{j}^{2}\frac{\sigma_{j}^{4}}{\lambda_{j}^{2}}+\tsum_{i=1}^{k}w_{j}^{2}\frac{\sigma_{j}^{4}}{\sigma_{i}^{2}}\frac{\lambda_{i}^{2}}{\lambda_{j}^{2}} & = & w_{j}^{2}\lambda_{j}^{-2}\sigma_{j}^{4}(1+\tsum_{i=1}^{k}\lambda_{i}^{2}\sigma_{i}^{-2})\\
...
% model $X^{*}=X\lambda_{i}(\lambda_{i}^{2}+\sigma_{i}^{2})^{-1/2}+\varepsilon$,
% which is congeneric with $\lambda^{*}=\lambda/(\lambda^{2}+\sigma^{2})^{-1/2}$
% and $\sigma^{*}=\sigma/(\lambda^{2}+\sigma^{2})^{-1/2}$. By the formula for the congeneric reliability \eqref{eq:Congeneric reliability}:
% \begin{eqnarray*}
% \omega_s & = & \frac{k\overline{\lambda^{*}}^{2}}{k\overline{\lambda^{*}}^{2}+\overline{\sigma^{\star2}}} = \frac{k\overline{\lambda(\lambda^{2}+\sigma^{2})^{-1/2}}^{2}}{k\overline{\lambda(\lambda^{2}+\sigma^{2})^{-1/2}}^{2}+\overline{\sigma^{2}/(\lambda^{2}+\sigma^{2})}}
% \end{eqnarray*}
% (iv) This one is similar to (ii). Just use the modified model $X^{*}=X\lambda\sigma^{-1}+\varepsilon$,
% which is congeneric with $\lambda^{*}=\lambda/\sigma^{-1}$ and
% $\sigma^{*}=1$. Again, by the formula for the congeneric reliability \eqref{eq:Congeneric reliability}:
% \begin{eqnarray*}
...
\[
\alpha_{s}=\frac{k}{k-1}\frac{i^{T}\Phi i-k}{i^{T}\Phi i}=k^{2}\frac{\overline{\rho}}{i^{T}\Phi i}.
\]
Since $(k\lambda^{2}+\sigma^{2})^{2}=(i^{T}\Phi i/k)^{2}$, 
\begin{equation}
\frac{\lambda^{2}}{(k\lambda^{2}+\sigma^{2})^{2}}\mathbf{i}^{T}\Xi\mathbf{i} =  k^{2}\frac{\overline{\rho}}{(i^{T}\Phi i)^{2}}\mathbf{i}^{T}\Xi\mathbf{i} =  \alpha_{s}\frac{\mathbf{i}^{T}\Xi\mathbf{i}}{\mathbf{i}^{T}\Psi\mathbf{i}},\nonumber
\end{equation}
as claimed.
