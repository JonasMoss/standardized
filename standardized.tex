...
\begin{equation}
\hat{X}_{i}\mid Y_i=-\frac{\phi(\tau_{iY_{i}})-\phi(\tau_{i(Y_{i}-1)})}{F(\tau_{iY_{i}})-F(\tau_{i(Y_{i}-1)})},\label{eq:Predictor of X}
\end{equation}
...
$\hat{\tau}$ into \eqref{eq:Xi element_1} and \eqref{eq:Xi element_2}. The sample estimator $\hat{\Xi}_{s}$ equals the sample covariance matrix of $\hat{X_{i}}$ when $\hat{X_{i}}$ is formed using the estimated thresholds $\hat{\tau}$. The estimators
