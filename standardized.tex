...
\label{appendix 2}
This proof is more involved than the other proofs of this paper. Here is an outline of the proof. First, modify the asymptotic variance for standardized alpha of \citet[][eq.9]{hayashi2005note} to account for non-normal variables. Second, modify the similar asymptotic variance of \citet{Van_Zyl2000-si} to work for non-normal variables. Third, do some algebra to show the asymptotic variances are equal under the parallel model.

In this appendix we prove the following proposition. Theorem 2 (ii)
is a corollary. 
\begin{prop}
...
\citep[see][Chapter 3.7]{Magnus2019-cz}, and $K_{d}$ is the diagonal
...
matrix $\Phi$.
\begin{description}
\item [{Coefficient~alpha}] The asympotic variance of cofficient alpha
is
\begin{equation}
\sigma^{2}=\frac{1}{(k-1)^{2}}\frac{1}{[\{(k-1)\rho+1]^{4}}\frac{1}{\phi^{4}}f^{T}\Gamma f,\label{eq:asymptotic variance}
\end{equation}
where $\Gamma$ is found in eq. (\ref{eq:Gamma}) and
\[
f=\vvec(\mathbf{i}\mathbf{i}^{T})=[(k-1)\rho+1]\vvec(I_{p})-\vvec(\mathbf{i}\mathbf{i}^{T}).
\]
\item [{Standardized~alpha}] The asymptotic variance of standardized alpha
is 
\begin{equation}
\sigma^{2}=\frac{1}{(k-1)^{2}}\frac{1}{[\{(k-1)\rho+1]^{4}}\frac{1}{\phi^{4}}\mathbf{i}_{*}^{T}D^{T}\Psi_{*}D\mathbf{i}_{*},\label{eq:asymptotic variance-1-1}
\end{equation}
...
\begin{proof}
(Coefficient alpha.) \citet{Van_Zyl2000-si} showed that, in the case
...
\end{eqnarray*}
Using Theorem 1 of \citet{Neudecker1990-ph}, formula (\ref{eq:zyl formula})
is easily generalized to non-normal variables. Using the substitution
$2(\Phi\otimes\Phi)=\Gamma$, the general formula is
...
(Standardized alpha.) Combining equations (8) and (9) of \citet{hayashi2005note},
