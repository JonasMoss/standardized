...
the power mean with exponent $p$. Then
...
\begin{proof}
Let's start with (\ref{eq:power mean inequality}). By the generalized
mean inequality $M_{p}\left(x+a\right)\leq\overline{x}+a$, and $M_{q}\left(x+a\right)\leq\overline{\frac{x}{x+a}}$,
...
by the generalized mean inequality, And since $f\left(x\right)\mapsto x/(x+a)$
is concave,
\[
