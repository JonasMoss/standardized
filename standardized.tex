...
Call the reliabiltiy with weights $w=\sigma^{-1}$ the \textit{sigma reliabiltiy} and denote it $\omega_\sigma$. Using Proposition \ref{prop:reliability motivation}, it can be written as
\begin{equation}
...
% as mentionioned, $\alpha_S=\omega_{s}$ only when $\lambda_{i}/(\lambda_{i}^{2}+\sigma_{i}^{2})=\lambda$.
% Otherwise, $\alpha_S<\omega_{s}$ by Propositon 1.
...
% Since $\omega$ has $\alpha$ and $\omega_{s}$ has $\alpha_S$,
